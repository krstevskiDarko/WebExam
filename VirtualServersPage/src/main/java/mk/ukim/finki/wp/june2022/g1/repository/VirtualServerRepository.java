...
public interface VirtualServerRepository extends JpaRepository<VirtualServer,Long> {
    List<VirtualServer> findAllByLaunchDateBefore(LocalDate filtered);
    List<VirtualServer> findAllByOwnersContaining(User user);
    List<VirtualServer> findAllByLaunchDateBeforeAndOwnersContaining(LocalDate filtered,User user);
}
